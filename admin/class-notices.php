...
class Notices {
	...
	public static function get_all() {
		...
		$notices = (array) json_decode( get_option( 'actax_admin_notices_' . get_current_user_id() ) );
	...
	private static function set( $notices ) {
		...
		if ( $notices === false ) {
			delete_option( 'actax_admin_notices_' . get_current_user_id() );
		} else {
			update_option( 'actax_admin_notices_' . get_current_user_id(), json_encode( $notices ) );
		}
