...
class Taxonomy {
	...
	public function save( $post ) {
		...
		if ( ! isset( $_POST ) || ! is_array( $_POST ) ) {
			throw new \Exception( 'No POST data to create custom taxonomy with.' );
		}

		$this->set_field_values( $post->ID );

		$this->post_data = (object) array(
			'taxonomy' => $this->field_values['post_type_name'],
			'args'      => $this->get_args()
		);
	...
	private function save_wp_post( $post_id ) {
		...
		wp_update_post( array(
			...
			'post_type'    => ACTAX_POST_TYPE,
			'post_name'    => 'actax_taxonomy_' . $this->field_values['post_type_name'],
	...
	private function save_wp_post_meta( $post_id ) {
		...
		foreach ( $field_names as $field_name ) {
			...
			update_post_meta( $post_id, 'actax_' . $field_name, $this->field_values[ $field_name ] );
		}
	}
	...
	private function save_json() {
		...
		if ( ! is_writable( $path ) ) {
			$this->errors[] = "The ACTAX JSON save path '$path' is not writable.";
	...
	/**
	 * Cleans up the actax field values
	 *
	 * @param $post_id
	 *
	 */
	private function set_field_values( $post_id ) {
		...
		foreach ( $this->fields->names() as $actax_field_name ) {

			$value = isset( $_POST[ $actax_field_name ] ) ? $_POST[ $actax_field_name ] : '';

			$name = substr( $actax_field_name, 6 );
		...
		$invalid_post_type_name_reason = $this->loader->is_invalid_taxonomy_name( $this->field_values['post_type_name'] );
		...
		foreach ( $this->get_unique_fields() as $key => $title ) {
			...
			if ( ! $this->is_unique( $post_id, $key, $value ) ) {
				$errors[] = "Another taxonomy has the same value '$value'. " .
	...
	/**
	 * creates the register_post_type arguments from the actax fields values
	 *
	 * @return array
	 */
	...
	private function get_field_filters() {
		return array(
			...
			'show_tagcloud'       => 'boolval',
			'show_in_admin_bar'   => 'boolval',
			'show_in_quick_edit'  => 'boolval',
	...
	public static function is_unique( $post_id, $field_name, $value ) {
		...
		$sql = $wpdb->prepare(
			"SELECT" . " COUNT(*) 
					...
					WHERE 1 = 1
					AND posts.ID != %d
					AND posts.post_type = 'actax_content_type'
					AND posts.post_status = 'publish'
					AND postmeta.meta_value = %s; ", "actax_$field_name", $post_id, $value );
