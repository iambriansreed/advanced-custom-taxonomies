...
class Meta_Boxes {
	...
	public function __construct( Fields $fields ) {
		...
		foreach ( $fields->groups() as $name => $field_group ) {
			...
			add_filter(
				'postbox_classes_' . ACTAX_POST_TYPE . '_' . $field_group->key,
	...
	public function set_field_values( $post ) {
		...
		$post_type_data = is_object( $post ) && $post->post_content ? json_decode( $post->post_content, true ) : false;
		...
		if ( $post_type_data && is_array( $post_type_data ) &&
			...
			foreach ( $post_type_data['args'] as $name => $value ) {
				$this->field_values[ 'actax_' . $name ] = $value;
			}
		} else {
			if ( $post && $post->ID ) {
				Notices::add_warning( 'There was an error loading your Custom Taxonomy.' );
			}
		}
